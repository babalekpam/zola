...
export default function JoinWorkspacePage() {
  ...
  async function onAccept() {
    ...
    try {
      ...
      localStorage.removeItem("loop_pending_org_invite");
      setActiveOrgId(org_id);
      ...
      <div className="w-full max-w-sm space-y-4 rounded-xl border border-border bg-card p-8 text-center">
        ...
        {preview.isLoading || authLoading ? (
          ...
          <>
            ...
            <p className="text-sm text-muted-foreground">
              This invite is no longer valid. Ask an admin for a new link.
            </p>
          </>
        ) : (
          <>
            <h1 className="text-xl font-semibold">
              Join {preview.data.org_name}
            </h1>
            <p className="text-sm text-muted-foreground">
              You've been invited to this workspace as a {preview.data.role}.
            </p>
            {user ? (
              <button
                onClick={onAccept}
