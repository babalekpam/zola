...
export function FileTree({ files, activePath, onSelect, onCreate, onDelete, onRename }: Props) {
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-border px-2 py-1.5">
        ...
        <button
          type="button"
          onClick={() => {
            const path = window.prompt("New file path (e.g. src/components/Button.tsx)");
...
function TreeNode({ node, depth, activePath, onSelect, onDelete, onRename }: {
  node: Node; depth: number; activePath: string | null;
  onSelect: (p: string) => void; onDelete: (p: string) => void; onRename: (f: string, t: string) => void;
}) {
  ...
  if (node.isFile) {
    return (
      <div
        className={cn(
          "group flex w-full items-center rounded px-2 py-1 hover:bg-accent cursor-pointer",
      ...
      >
        <button type="button" onClick={() => onSelect(node.path)} className="flex flex-1 items-center gap-1.5 text-left">
          <File className="h-3.5 w-3.5 opacity-60 shrink-0" />
  ...
  return (
    <div>
      <div
        ...
        style={{ paddingLeft: depth * 12 + 8 }}
      >
        <FolderOpen className="h-3.5 w-3.5 opacity-60 shrink-0" />
        <span className="truncate">{node.name}</span>
      </div>
      {node.children.map((c) => (
        <TreeNode
